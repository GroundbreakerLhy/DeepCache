...
def dataset(np_dir="./pin_dataset/pin_dataset/", compiler="tvm", paral_ver=False):
    ...
    if not mat_mode:
        trainset = CacheDataset.LargeCachePicDataset(
            np_dir, compiler=compiler, paral_ver=paral_ver
        )
        trainloader = torch.utils.data.DataLoader(
            trainset, batch_size=batch_size, shuffle=True, num_workers=4, drop_last=True
        )
        testset = CacheDataset.LargeCachePicDataset(
            np_dir, train=False, compiler=compiler
        )
        ...
        testset = CacheDataset.LargeCacheMatDataset(
            np_dir, train=False, compiler=compiler
        )
        testloader = torch.utils.data.DataLoader(
            testset,
...
def database(np_dir="./pin_dataset/pin_dataset/", compiler="tvm"):
    ...
    if not mat_mode:
        trainset = CacheDataset.LargeCachePicDataset(
            np_dir, compiler=compiler, as_database=True
        )
        trainloader = torch.utils.data.DataLoader(
            trainset, batch_size=batch_size, shuffle=True, num_workers=4, drop_last=True
        )
        testset = CacheDataset.LargeCachePicDataset(
            np_dir, train=False, compiler=compiler, as_database=True
        )
        testloader = torch.utils.data.DataLoader(
            testset,
            batch_size=batch_size,
...
def train_step(net, criterion, optimizer, epoch, trainloader):
    ...
    for batch_idx, (inputs1, inputs2, targets, indexes) in pbar:
        data_time.update(time.time() - end)

        # print(targets)
        # input("continue?")
        inputs1, inputs2, indexes = (
            inputs1.to(device),
            inputs2.to(device),
            indexes.to(device),
        )
...
def train(net, criterion, optimizer, trainloader, testloader, start_epoch=0):
    ...
    pbar_epoch = tqdm(range(start_epoch, start_epoch + epoch), desc="Training")
    for epoch_num in pbar_epoch:
        net, criterion, optimizer, train_loss = train_step(
            net, criterion, optimizer, epoch_num, trainloader
        )

        pbar_epoch.set_postfix(
            {"best_loss": f"{best_loss:.4f}", "curr_loss": f"{train_loss:.4f}"}
        )
...
def generate_embedding_database(
    net, trainset, embedding_file="../pin_dataset/pin_dataset_glow/embeddings.json"
):
    global model_name
...
def database_matching(
    ...
    def trace_similarity_LLC(
        test_embeddings, target_embeddings, input_label="", target_label=""
    ):
    ...
    def trace_similarity(
        ...
        if LLC:
            return trace_similarity_LLC(
                test_embeddings, target_embeddings, input_label, target_label
            )
        else:
            return trace_similarity_internal(
                test_embeddings, target_embeddings, input_label, target_label
            )
    ...
    for test_id in range(testset.name_id):
        ...
        actual_topk = min(topk, len(sim_values))
        yd, yi = sim_values.topk(actual_topk, dim=0, largest=True, sorted=True)  # TOP K
        # print(yd, yi)
        # input("debug")
        ...
        for idx in range(len(yi)):
            ...
            output_labels.append(
                (trainset.id2name[label_id.item()], sim_val.item())
            )  # key should be ints
